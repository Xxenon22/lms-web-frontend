    ...
    try {
        ...
        profileGuru.value = (res.data.profiles || []).map(data => ({
            ...
            photo_profiles_user: data.photo_profiles_user
                ? `${import.meta.env.VITE_API_URL}/api${data.photo_profiles_user}`
                : null
        }));
...
<template>
    <div class="flex justify-between m-3">
        <h1 class="font-bold">Class Learning Plan</h1>
        <AutoComplete v-model="searchText" optionLabel="username" :suggestions="suggestions" @complete="searchGuru"
            @item-select="onSelectGuru" @clear="onClear" placeholder="Search teacher..." />
    </div>

    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
        <div v-for="profile in filteredGuru" :key="profile.id">
            <RouterLink :to="{ name: 'CLP', params: { id: profile.id } }">
                <Button class="w-64">
