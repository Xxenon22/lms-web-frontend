...
const uploadFile = async () => {
    ...
    try {
        ...
        const res = await api.post("/timetables-grade-x", formData, {
            headers: { "Content-Type": "multipart/form-data" },
        });

        fileUrl.value = `${import.meta.env.VITE_API_URL}/uploads/timetables-grade-x/${res.data.file_name}`;
        uploaded.value = true;
...
const fetchFiles = async () => {
    try {
        ...
        files.value = res.data.map((file) => ({
            id: file.id,
            name: file.file_name,
            url: `${import.meta.env.VITE_API_URL}/api/uploads/timetables-grade-x/${file.file_name}`,
        }));
    } catch (err) {
...
const deleteFile = async (id) => {
    try {
        await api.delete(`/timetables-grade-x/${id}`);
